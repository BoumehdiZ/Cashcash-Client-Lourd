...
public final class ConstraintParser
{
    ...
    private static DimConstraint parseDimConstraint(final String s, final BoundSize gapBefore, final BoundSize gapAfter, final boolean b) {
        ...
        final String[] trimmedTokens = toTrimmedTokens(s, ',');
        for (int i = 0; i < trimmedTokens.length; ++i) {
            final String str = trimmedTokens[i];
            try {
                if (str.length() != 0) {
                    if (str.equals("fill")) {
                        ...
                        if (char1 == 's') {
                            final int startsWithLenient = startsWithLenient(str, new String[] { "sizegroup", "sg" }, new int[] { 5, 2 }, true);
                            if (startsWithLenient > -1) {
                                dimConstraint.setSizeGroup(str.substring(startsWithLenient).trim());
                                continue;
                            }
                            final int startsWithLenient2 = startsWithLenient(str, new String[] { "shrinkprio", "shp" }, new int[] { 10, 3 }, true);
                            if (startsWithLenient2 > -1) {
                                dimConstraint.setShrinkPriority(Integer.parseInt(str.substring(startsWithLenient2).trim()));
                                continue;
                            }
                            final int startsWithLenient3 = startsWithLenient(str, "shrink", 6, true);
                            if (startsWithLenient3 > -1) {
                                dimConstraint.setShrink(parseFloat(str.substring(startsWithLenient3).trim(), ResizeConstraint.WEIGHT_100));
                                continue;
                            }
                        }
                        if (char1 == 'g') {
                            final int startsWithLenient4 = startsWithLenient(str, new String[] { "growpriority", "gp" }, new int[] { 5, 2 }, true);
                            if (startsWithLenient4 > -1) {
                                dimConstraint.setGrowPriority(Integer.parseInt(str.substring(startsWithLenient4).trim()));
                                continue;
                            }
                            final int startsWithLenient5 = startsWithLenient(str, "grow", 4, true);
                            if (startsWithLenient5 > -1) {
                                dimConstraint.setGrow(parseFloat(str.substring(startsWithLenient5).trim(), ResizeConstraint.WEIGHT_100));
                                continue;
                            }
                        }
                        if (char1 == 'a') {
                            final int startsWithLenient6 = startsWithLenient(str, "align", 2, true);
                            if (startsWithLenient6 > -1) {
                                dimConstraint.setAlign(parseUnitValueOrAlign(str.substring(startsWithLenient6).trim(), b, null));
                                continue;
                            }
                        }
                        final UnitValue alignKeywords = parseAlignKeywords(str, b);
                        if (alignKeywords != null) {
                            dimConstraint.setAlign(alignKeywords);
                        }
                        else {
                            dimConstraint.setSize(parseBoundSize(str, false, b));
                        }
                    }
                }
            }
    ...
    public static UnitValue[] parseInsets(final String str, final boolean b) {
        if (str.length() != 0 && !str.equals("dialog") && !str.equals("panel")) {
            final String[] trimmedTokens = toTrimmedTokens(str, ' ');
        ...
        final boolean startsWith = str.startsWith("p");
    ...
    private static BoundSize[] parseGaps(String str) {
        ...
        final int startsWithLenient = startsWithLenient(str, "gaptop", -1, true);
        if (startsWithLenient > -1) {
            str = str.substring(startsWithLenient).trim();
            array[0] = parseBoundSize(str, true, false);
            return array;
        }
        final int startsWithLenient2 = startsWithLenient(str, "gapleft", -1, true);
        if (startsWithLenient2 > -1) {
            str = str.substring(startsWithLenient2).trim();
            array[1] = parseBoundSize(str, true, true);
            return array;
        }
        final int startsWithLenient3 = startsWithLenient(str, "gapbottom", -1, true);
        if (startsWithLenient3 > -1) {
            str = str.substring(startsWithLenient3).trim();
            array[2] = parseBoundSize(str, true, false);
            return array;
        }
        final int startsWithLenient4 = startsWithLenient(str, "gapright", -1, true);
        if (startsWithLenient4 > -1) {
            str = str.substring(startsWithLenient4).trim();
            array[3] = parseBoundSize(str, true, true);
            return array;
        }
        final int startsWithLenient5 = startsWithLenient(str, "gapbefore", -1, true);
        if (startsWithLenient5 > -1) {
            str = str.substring(startsWithLenient5).trim();
            array[1] = parseBoundSize(str, true, true);
            return array;
        }
        final int startsWithLenient6 = startsWithLenient(str, "gapafter", -1, true);
        if (startsWithLenient6 > -1) {
            str = str.substring(startsWithLenient6).trim();
            array[3] = parseBoundSize(str, true, true);
            return array;
        }
        final int startsWithLenient7 = startsWithLenient(str, new String[] { "gapx", "gapy" }, null, true);
        if (startsWithLenient7 > -1) {
            final int n = (str.charAt(3) == 'x') ? 1 : 0;
            final String[] trimmedTokens = toTrimmedTokens(str.substring(startsWithLenient7).trim(), ' ');
        ...
        final int startsWithLenient8 = startsWithLenient(str, "gap ", 1, true);
        if (startsWithLenient8 > -1) {
            final String[] trimmedTokens2 = toTrimmedTokens(str.substring(startsWithLenient8).trim(), ' ');
    ...
    public static BoundSize parseBoundSize(String substring, final boolean b, final boolean b2) {
        ...
        if (trimmedTokens.length == 1) {
            ...
            return new BoundSize((b || endsWith) ? unitValue : null, unitValue, endsWith ? unitValue : null, b3, str);
        }
        if (trimmedTokens.length == 2) {
            return new BoundSize(parseUnitValue(substring2, null, b2), parseUnitValue(trimmedTokens[1], null, b2), null, b3, str);
        }
        if (trimmedTokens.length == 3) {
            return new BoundSize(parseUnitValue(substring2, null, b2), parseUnitValue(trimmedTokens[1], null, b2), parseUnitValue(trimmedTokens[2], null, b2), b3, str);
        }
    ...
    private static String[] getNumTextParts(final String str) {
