...
public final class LinkHandler
{
    ...
    static synchronized boolean setBounds(final Object referent, final String s, final int n, final int n2, final int n3, final int n4, final boolean b, final boolean b2) {
        int i = LinkHandler.LAYOUTS.size() - 1;
        while (i >= 0) {
            if (LinkHandler.LAYOUTS.get(i).get() == referent) {
        ...
        LinkHandler.LAYOUTS.add(new WeakReference<Object>(referent));
        final int[] array2 = { n, n2, n3, n4, n + n3, n2 + n4 };
        final HashMap<String, int[]> e = new HashMap<String, int[]>(4);
        if (b) {
            e.put(s, array2);
        }
        LinkHandler.VALUES_TEMP.add(e);
        final HashMap<String, int[]> e2 = new HashMap<String, int[]>(4);
        if (!b) {
            e2.put(s, array2);
        }
        LinkHandler.VALUES.add(e2);
        return true;
    }
    ...
    public static synchronized boolean clearBounds(final Object o, final String key) {
        for (int i = LinkHandler.LAYOUTS.size() - 1; i >= 0; --i) {
            if (LinkHandler.LAYOUTS.get(i).get() == o) {
                return LinkHandler.VALUES.get(i).remove(key) != null;
            }
        }
        return false;
    }
