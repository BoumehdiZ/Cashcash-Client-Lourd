...
public final class PlatformDefaults
{
    ...
    public static void setPlatform(final int n) {
        switch (n) {
            case 0: {
        ...
        PlatformDefaults.CUR_PLAF = n;
        PlatformDefaults.BASE_DPI = ((PlatformDefaults.BASE_DPI_FORCED != null) ? PlatformDefaults.BASE_DPI_FORCED : getPlatformDPI(n));
    }
    
    private static int getPlatformDPI(final int i) {
        switch (i) {
            case 0:
            case 2: {
                return 96;
    ...
    public static void setLogicalPixelBase(final int n) {
        if (PlatformDefaults.LP_BASE != n) {
            if (n < 100 || n > 101) {
                throw new IllegalArgumentException("Unrecognized base: " + n);
            }
            PlatformDefaults.LP_BASE = n;
            ++PlatformDefaults.MOD_COUNT;
        }
    }
    ...
    public static UnitValue getUnitValueX(final String key) {
        return PlatformDefaults.HOR_DEFS.get(key);
    }
    
    public static UnitValue getUnitValueY(final String key) {
        return PlatformDefaults.VER_DEFS.get(key);
    }
    
    public static final void setUnitValue(final String[] array, final UnitValue value, final UnitValue value2) {
        for (int i = 0; i < array.length; ++i) {
            final String trim = array[i].toLowerCase().trim();
            if (value != null) {
                PlatformDefaults.HOR_DEFS.put(trim, value);
            }
            if (value2 != null) {
                PlatformDefaults.VER_DEFS.put(trim, value2);
            }
        }
        ++PlatformDefaults.MOD_COUNT;
    }
    
    static final int convertToPixels(final float n, final String key, final boolean b, final float n2, final ContainerWrapper containerWrapper, final ComponentWrapper componentWrapper) {
        final UnitValue unitValue = (b ? PlatformDefaults.HOR_DEFS : PlatformDefaults.VER_DEFS).get(key);
    ...
    public static final void setDefaultHorizontalUnit(final int n) {
        if (n < 0 || n > 27) {
            throw new IllegalArgumentException("Illegal Unit: " + n);
        }
        if (PlatformDefaults.DEF_H_UNIT != n) {
            PlatformDefaults.DEF_H_UNIT = n;
            ++PlatformDefaults.MOD_COUNT;
        }
    }
    ...
    public static final void setDefaultVerticalUnit(final int n) {
        if (n < 0 || n > 27) {
            throw new IllegalArgumentException("Illegal Unit: " + n);
        }
        if (PlatformDefaults.DEF_V_UNIT != n) {
            PlatformDefaults.DEF_V_UNIT = n;
            ++PlatformDefaults.MOD_COUNT;
        }
    }
