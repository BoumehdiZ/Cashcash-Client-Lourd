...
public class SwingComponentWrapper implements ComponentWrapper
{
    ...
    public final float getPixelUnitFactor(final boolean b) {
        switch (PlatformDefaults.getLogicalPixelBase()) {
            case 100: {
                ...
                Point2D.Float value = SwingComponentWrapper.FM_MAP.get(fontMetrics);
                if (value == null) {
                    final Rectangle2D stringBounds = fontMetrics.getStringBounds("X", this.c.getGraphics());
                    value = new Point2D.Float((float)stringBounds.getWidth() / 6.0f, (float)stringBounds.getHeight() / 13.277344f);
                    SwingComponentWrapper.FM_MAP.put(fontMetrics, value);
                }
                return b ? value.x : value.y;
            }
            case 101: {
    ...
    public final int getPreferredHeight(final int width) {
        if (this.c.getWidth() == 0 && this.c.getHeight() == 0 && width != -1) {
            this.c.setBounds(this.c.getX(), this.c.getY(), width, 1);
        }
        return this.c.getPreferredSize().height;
    }
    
    public final int getPreferredWidth(final int height) {
        if (this.c.getWidth() == 0 && this.c.getHeight() == 0 && height != -1) {
            this.c.setBounds(this.c.getX(), this.c.getY(), 1, height);
        }
    ...
    private boolean isMaxSet(final Component obj) {
        if (SwingComponentWrapper.IMS_METHOD != null) {
            try {
                return (boolean)SwingComponentWrapper.IMS_METHOD.invoke(obj, (Object[])null);
            }
            catch (Exception ex) {
